...
export default async function AdminDashboard() {
  ...
  try {
    ...
    const totalPaidCount = await prisma.eventPlayer.count({
      where: { paymentStatus: "FULLY_PAID" },
    });
    const totalPendingReports = await prisma.errorReport.count({
      where: { status: "pending" },
    });

    // Calculate accurate unpaid amount
    const eventPlayersWithDebt = await prisma.eventPlayer.findMany({
      where: {
        paymentStatus: {
